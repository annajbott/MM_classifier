...
naive_pretrain <- readRDS("MM_naive_scclassify_model.rds")
relapse_pretrain <- readRDS("MM_relapsed_scclassify_model.rds")

list_train <- scClassifyTrainModelList(naive_pretrain, relapse_pretrain)
...
pred_res <- predict_scClassifyJoint(exprsMat_test = dgc_mat,
                             trainRes = list_train, # or pre-made reference using
                             cellTypes_test = NULL,
                             algorithm = "WKNN",
                             features = c("limma"),
...
seurat_object@meta.data[['MM_joint_naive_relapse_labels']] <- cells_assigned
